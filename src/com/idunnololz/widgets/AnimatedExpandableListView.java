...
public class AnimatedExpandableListView extends ExpandableListView {
	...
	public final View getChildView(final int groupPosition,
		...
		int totalHeight = 0;
		final int clipHeight = parent.getHeight();

		final int len = getRealChildrenCount(groupPosition);
		for (int i = info.firstChildPosition; i < len; i++) {
		    final View childView = getRealChildView(groupPosition, i,
			    (i == len - 1), null, parent);
			...
			final int averageHeight = totalHeight / (i + 1);
		...
		Object o;
		final int state = (o = dummyView.getTag()) == null ? STATE_IDLE
			: (Integer) o;

		if (info.expanding && state != STATE_EXPANDING) {
		    final ExpandAnimation ani = new ExpandAnimation(dummyView,
			    0, totalHeight, info);
	...
	protected void applyTransformation(float interpolatedTime,
		...
		final int val = baseHeight + (int) (delta * interpolatedTime);
		view.getLayoutParams().height = val;
		groupInfo.dummyHeight = val;
		view.requestLayout();
	    } else {
		final int val = baseHeight + delta;
		view.getLayoutParams().height = val;
		groupInfo.dummyHeight = val;
		view.requestLayout();
	    }
	...
	final int groupFlatPos = getFlatListPosition(getPackedPositionForGroup(groupPos));
	if (groupFlatPos != -1) {
	    final int childIndex = groupFlatPos - getFirstVisiblePosition();
	...
	final int firstGroupPos = getPackedPositionGroup(packedPos);
	...
	final boolean lastGroup = groupPos == adapter.getGroupCount() - 1;
	if (lastGroup
